...
public class Dessourt121Combat extends NCombatScript {
    @Override
    public List<NpcCombatDefinition> getCombatDefs() {
        var combat = NpcCombatDefinition.builder();
        combat.id(NpcId.DESSOURT_121);
        combat.hitpoints(NpcCombatHitpoints.total(130));
        combat.stats(NpcCombatStats.builder().attackLevel(99).defenceLevel(99).bonus(CombatBonus.MELEE_ATTACK, 50).bonus(CombatBonus.DEFENCE_STAB, 10).bonus(CombatBonus.DEFENCE_SLASH, 150).bonus(CombatBonus.DEFENCE_CRUSH, 150).build());
