...
public class VeracTheDefiled115Combat extends NCombatScript {
    @Override
    public List<NpcCombatDefinition> getCombatDefs() {
        ...
        combat.id(NpcId.VERAC_THE_DEFILED_115);
        combat.spawn(NpcCombatSpawn.builder().respawnDelay(75).build());
        combat.hitpoints(NpcCombatHitpoints.total(100));
        combat.stats(NpcCombatStats.builder().attackLevel(100).defenceLevel(100).bonus(CombatBonus.ATTACK_STAB, 68).bonus(CombatBonus.ATTACK_CRUSH, 82).bonus(CombatBonus.DEFENCE_STAB, 227).bonus(CombatBonus.DEFENCE_SLASH, 230).bonus(CombatBonus.DEFENCE_CRUSH, 221).bonus(CombatBonus.DEFENCE_RANGED, 225).build());
        combat.killCount(NpcCombatKillCount.builder().asName("Barrows brother").build());
        combat.combatScript("VeracCS").deathAnimation(836).blockAnimation(2063);
