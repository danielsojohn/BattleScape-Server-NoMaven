...
public class ToragTheCorrupted115Combat extends NCombatScript {
    @Override
    public List<NpcCombatDefinition> getCombatDefs() {
        ...
        combat.id(NpcId.TORAG_THE_CORRUPTED_115);
        combat.spawn(NpcCombatSpawn.builder().respawnDelay(75).build());
        combat.hitpoints(NpcCombatHitpoints.total(100));
        combat.stats(NpcCombatStats.builder().attackLevel(100).defenceLevel(100).bonus(CombatBonus.ATTACK_STAB, 68).bonus(CombatBonus.ATTACK_CRUSH, 82).bonus(CombatBonus.DEFENCE_STAB, 221).bonus(CombatBonus.DEFENCE_SLASH, 235).bonus(CombatBonus.DEFENCE_CRUSH, 222).bonus(CombatBonus.DEFENCE_RANGED, 221).build());
        combat.killCount(NpcCombatKillCount.builder().asName("Barrows brother").build());
        combat.combatScript("ToragCS").deathAnimation(836).blockAnimation(424);
