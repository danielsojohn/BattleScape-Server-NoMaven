...
public class AgrithNaarHard196Combat extends NpcCombat {
    @Override
    public List<NpcCombatDefinition> getCombatDefinitions() {
        var drop = NpcCombatDrop.builder();
        var dropTable = NpcCombatDropTable.builder().chance(NpcCombatDropTable.CHANCE_ALWAYS);
        dropTable.drop(NpcCombatDropTableDrop.items(new RandomItem(ItemId.DARKLIGHT)));
        drop.table(dropTable.build());


        var combat = NpcCombatDefinition.builder();
        combat.id(NpcId.AGRITH_NAAR_HARD_196);
        combat.hitpoints(NpcCombatHitpoints.total(209));
        combat.stats(NpcCombatStats.builder().attackLevel(182).magicLevel(220).rangedLevel(220).defenceLevel(82).build());
