...
public class CursedHydra400_16012Combat extends NCombatScript {
    @Override
    public List<NpcCombatDefinition> getCombatDefs() {
        var drop = NpcCombatDrop.builder();
        var dropTable = NpcCombatDropTable.builder().chance(0.049).log(true);
        dropTable.drop(NpcCombatDropTableDrop.items(new RandomItem(ItemId.HYDRAS_CLAW)));
        drop.table(dropTable.build());
        dropTable = NpcCombatDropTable.builder().chance(0.09).log(true);
        ...
        drop.table(dropTable.build());
        dropTable = NpcCombatDropTable.builder().chance(0.42).log(true);
        dropTable.drop(NpcCombatDropTableDrop.items(new RandomItem(ItemId.HYDRA_TAIL)));
        drop.table(dropTable.build());
        dropTable = NpcCombatDropTable.builder().chance(NpcCombatDropTable.CHANCE_1_IN_128);
        dropTable.drop(NpcCombatDropTableDrop.items(new RandomItem(ItemId.CLUE_SCROLL_HARD)));
        drop.table(dropTable.build());
        dropTable = NpcCombatDropTable.builder().chance(1.0).order(NpcCombatDropTable.Order.RANDOM_UNIQUE).log(true);
        ...
        combat.id(NpcId.CURSED_HYDRA_400_16012);
        combat.hitpoints(NpcCombatHitpoints.builder().total(400).bar(HitpointsBar.GREEN_RED_60).build());
        combat.stats(NpcCombatStats.builder().attackLevel(80).magicLevel(210).rangedLevel(210).defenceLevel(100).bonus(CombatBonus.MELEE_DEFENCE, 160).bonus(CombatBonus.DEFENCE_MAGIC, 160).build());
        ...
        combat.combatScript("cursedhydra").deathAnimation(8257);
