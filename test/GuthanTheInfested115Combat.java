...
public class GuthanTheInfested115Combat extends NCombatScript {
    @Override
    public List<NpcCombatDefinition> getCombatDefs() {
        ...
        combat.id(NpcId.GUTHAN_THE_INFESTED_115);
        combat.spawn(NpcCombatSpawn.builder().respawnDelay(75).build());
        combat.hitpoints(NpcCombatHitpoints.total(100));
        combat.stats(NpcCombatStats.builder().attackLevel(100).defenceLevel(100).bonus(CombatBonus.ATTACK_STAB, 75).bonus(CombatBonus.ATTACK_SLASH, 75).bonus(CombatBonus.ATTACK_CRUSH, 75).bonus(CombatBonus.DEFENCE_STAB, 259).bonus(CombatBonus.DEFENCE_SLASH, 257).bonus(CombatBonus.DEFENCE_CRUSH, 241).bonus(CombatBonus.DEFENCE_MAGIC, -11).bonus(CombatBonus.DEFENCE_RANGED, 250).build());
        combat.killCount(NpcCombatKillCount.builder().asName("Barrows brother").build());
        combat.combatScript("GuthanCS").deathAnimation(836).blockAnimation(435);
