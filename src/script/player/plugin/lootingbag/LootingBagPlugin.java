...
import com.palidino.osrs.model.item.ItemList;
...
import script.player.plugin.lootingbag.dialogue.StoreAskDialogue;
import script.player.plugin.lootingbag.dialogue.StoreTypeDialogue;
...
public class LootingBagPlugin extends PlayerPlugin {
    ...
    public void storeItemFromInventory(int itemSlot, StoreType storeType) {
        ...
        if (storeType == StoreType.STORE_1) {
            ...
            amount = ItemDef.getStackOrNote(itemId) ? player.getInventory().getAmount(itemSlot) : Item.MAX_AMOUNT;
        }
    ...
    public void bankItems() {
        while (!getItems().isEmpty()) {
            ...
            if (!player.getBank().deposit(getItems(), getItems().getId(slot), slot, getItems().getAmount(slot))) {
                break;
            }
        }
    }
