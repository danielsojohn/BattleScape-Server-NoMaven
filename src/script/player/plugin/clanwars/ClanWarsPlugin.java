...
import lombok.var;
import script.player.plugin.clanwars.dialogue.JoinBattleDialogue;
import script.player.plugin.clanwars.rule.Arena;
import script.player.plugin.clanwars.rule.Rule;
import script.player.plugin.clanwars.rule.RuleOption;
import script.player.plugin.clanwars.state.BarrierState;
...
public class ClanWarsPlugin extends PlayerPlugin {
    ...
    public void tick() {
        if (inClanWarsChallengeArea != player.inClanWarsChallengeArea()) {
            ...
            player.getGameEncoder().sendPlayerOption(inClanWarsChallengeArea ? "Challenge" : "null", 1, false);
        }
        var inClanWarsTournamentStatusArea = player.inClanWarsTournamentStatusArea();
        if (inTournamentPlayerStateArea || inTournamentPlayerStateArea != inClanWarsTournamentStatusArea) {
    ...
    public boolean playerOptionHook(int index, Player player2) {
        if (index == 0 && player.inClanWarsChallengeArea() && player2.inClanWarsChallengeArea()) {
            if (state != PlayerState.NONE || player2.getPlugin(ClanWarsPlugin.class).getState() != PlayerState.NONE) {
                return true;
            }
    ...
    public void loadBarrier(BarrierState stage) {
        ...
        for (var i = getArena().getBarrierStartX(); i <= getArena().getBarrierEndX(); i++) {
            ...
            var barrier = new MapObject(getArena().getBarrierObjectId(), i, getArena().getBarrierY(), 0, 10,
            ...
            if (stage != BarrierState.DELETE && current != null && current.getId() != getArena().getBarrierObjectId()
    ...
    public void changeRule(Rule rule, int slot) {
        ...
        if (opponent.getPlugin(ClanWarsPlugin.class).getState() != PlayerState.RULE_SELECTION
                && opponent.getPlugin(ClanWarsPlugin.class).getState() != PlayerState.ACCEPT_RULE_SELECTION) {
            return;
        }
