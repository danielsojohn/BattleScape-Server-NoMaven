package script.player.plugin.clanwars;

import com.palidino.osrs.io.cache.ScriptId;
...
import script.player.plugin.clanwars.state.CompletedState;
...
public class Stages {
    public static void openRuleSelection(Player player, ClanWarsPlugin plugin) {
        ...
        player.getWidgetManager().sendInteractiveOverlay(WidgetId.CLAN_WARS_OPTIONS, new WidgetManager.CloseEvent() {
            @Override
            public void execute() {
                ...
                opponentPlayer.getGameEncoder().sendMessage(player.getUsername() + " declined the war.");
    ...
    public static void openRuleConfirmation(Player player, ClanWarsPlugin plugin) {
        ...
        player.getWidgetManager().sendInteractiveOverlay(WidgetId.CLAN_WARS_CONFIRM, new WidgetManager.CloseEvent() {
            @Override
            public void execute() {
                var opponentPlayer = plugin.getOpponent();
                plugin.cancel();
    ...
    public static void acceptRuleConfirmation(Player player, ClanWarsPlugin plugin) {
        ...
        if ((player.carryingMembersItems() || plugin.getOpponent().carryingMembersItems())
            ...
            player.getGameEncoder().sendMessage("Members items can't be taken with the F2P Content rule set.");
        ...
        if (player.getWorld().getPlayerInstance(player, "clan_wars_battle") != null) {
            player.getGameEncoder().sendMessage("There is already a clan wars instance for this Clan Chat.");
            plugin.getOpponent().getGameEncoder()
        ...
        if (player.getWorld().getPlayerInstance(plugin.getOpponent(), "clan_wars_battle") != null) {
            player.getGameEncoder().sendMessage("There is already a clan wars instance for their Clan Chat.");
            plugin.getOpponent().getGameEncoder()
    ...
    public static void acceptJoin(Player player, ClanWarsPlugin plugin) {
        ...
        var opponentPlugin = plugin.getOpponent().getPlugin(ClanWarsPlugin.class);
        if (!player.getMessaging().matchesClanChat(plugin.getTeammate().getMessaging().getClanChatUsername())
                || opponentPlugin.getState() != PlayerState.BATTLE) {
            player.getGameEncoder().sendMessage("Error joining war.");
            player.getWidgetManager().removeInteractiveWidgets();
            return;
        }
        if (plugin.ruleSelected(Rule.GAME_END, RuleOption.LAST_TEAM_STANDING) && opponentPlugin.getCountdown() == 0) {
            player.getGameEncoder().sendMessage("The war has already started.");
            player.getWidgetManager().removeInteractiveWidgets();
            return;
        }
        if (plugin.ruleSelected(Rule.IGNORE_FREEZING, RuleOption.ALLOWED) && player.carryingMembersItems()) {
    ...
    public static void acceptViewing(Player player, ClanWarsPlugin plugin) {
        if (plugin.getTeammate() == null || plugin.getTeammate().isLocked()
                ...
                || !player.getMessaging().matchesClanChat(plugin.getTeammate().getMessaging().getClanChatUsername())
