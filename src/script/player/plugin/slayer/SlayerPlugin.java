...
import com.palidino.osrs.model.player.slayer.AssignedSlayerTask;
...
public class SlayerPlugin extends PlayerPlugin {
    private static final int[] BLOCKED_TASK_VARBITS = {
        VarbitId.SLAYER_BLOCKED_TASK_1, VarbitId.SLAYER_BLOCKED_TASK_2, VarbitId.SLAYER_BLOCKED_TASK_3,
        VarbitId.SLAYER_BLOCKED_TASK_4, VarbitId.SLAYER_BLOCKED_TASK_5, VarbitId.SLAYER_BLOCKED_TASK_6
    };
    private static final List<RandomItem> BRIMSTONE_CHEST_ITEMS =
            RandomItem.buildList(new RandomItem(ItemId.UNCUT_DIAMOND, 25, 35).setWeight(1024 - 12),
                    new RandomItem(ItemId.UNCUT_RUBY, 25, 35).setWeight(1024 - 12),
                    new RandomItem(ItemId.COAL, 300, 500).setWeight(1024 - 12),
                    new RandomItem(ItemId.COINS, 50_000, 150_000).setWeight(1024 - 12),
                    new RandomItem(ItemId.GOLD_ORE, 100, 200).setWeight(1024 - 15),
                    new RandomItem(ItemId.DRAGON_ARROWTIPS, 50, 200).setWeight(1024 - 15),
                    new RandomItem(ItemId.IRON_ORE, 350, 500).setWeight(1024 - 20),
                    new RandomItem(ItemId.RUNE_FULL_HELM, 2, 4).setWeight(1024 - 20),
                    new RandomItem(ItemId.RUNE_PLATEBODY, 1, 2).setWeight(1024 - 20),
                    new RandomItem(ItemId.RUNE_PLATELEGS, 1, 2).setWeight(1024 - 20),
                    new RandomItem(ItemId.RAW_TUNA, 100, 350).setWeight(1024 - 20),
                    new RandomItem(ItemId.RAW_LOBSTER, 100, 350).setWeight(1024 - 20),
                    new RandomItem(ItemId.RAW_SWORDFISH, 100, 350).setWeight(1024 - 20),
                    new RandomItem(ItemId.RAW_MONKFISH, 100, 300).setWeight(1024 - 20),
                    new RandomItem(ItemId.RAW_SHARK, 100, 250).setWeight(1024 - 20),
                    new RandomItem(ItemId.RAW_SEA_TURTLE, 80, 200).setWeight(1024 - 20),
                    new RandomItem(ItemId.RAW_MANTA_RAY, 80, 160).setWeight(1024 - 20),
                    new RandomItem(ItemId.RUNITE_ORE, 10, 15).setWeight(1024 - 30),
                    new RandomItem(ItemId.STEEL_BAR, 300, 500).setWeight(1024 - 30),
                    new RandomItem(ItemId.MAGIC_LOGS, 120, 160).setWeight(1024 - 30),
                    new RandomItem(ItemId.DRAGON_DART_TIP, 40, 160).setWeight(1024 - 30),
                    new RandomItem(ItemId.PALM_TREE_SEED, 2, 4).setWeight(1024 - 60),
                    new RandomItem(ItemId.MAGIC_SEED, 2, 4).setWeight(1024 - 60),
                    new RandomItem(ItemId.CELASTRUS_SEED, 2, 4).setWeight(1024 - 60),
                    new RandomItem(ItemId.DRAGONFRUIT_TREE_SEED, 2, 4).setWeight(1024 - 60),
                    new RandomItem(ItemId.REDWOOD_TREE_SEED).setWeight(1024 - 60),
                    new RandomItem(ItemId.TORSTOL_SEED, 3, 5).setWeight(1024 - 60),
                    new RandomItem(ItemId.SNAPDRAGON_SEED, 3, 5).setWeight(1024 - 60),
                    new RandomItem(ItemId.RANARR_SEED, 3, 5).setWeight(1024 - 60),
                    new RandomItem(ItemId.PURE_ESSENCE, 3000, 6000).setWeight(1024 - 60),
                    new RandomItem(ItemId.DRAGON_HASTA).setWeight(1024 - 200),
                    new RandomItem(ItemId.MYSTIC_HAT_DUSK).setWeight(1024 - 1000),
                    new RandomItem(ItemId.MYSTIC_ROBE_TOP_DUSK).setWeight(1024 - 1000),
                    new RandomItem(ItemId.MYSTIC_ROBE_BOTTOM_DUSK).setWeight(1024 - 1000),
                    new RandomItem(ItemId.MYSTIC_GLOVES_DUSK).setWeight(1024 - 1000),
                    new RandomItem(ItemId.MYSTIC_BOOTS_DUSK).setWeight(1024 - 1000));
    ...
    public void loadLegacy(Map<String, Object> map) {
        ...
        if (map.containsKey("slayer.wildernessTaskAmount")) {
            wildernessTask = new AssignedSlayerTask((String) map.get("slayer.wildernessTaskMaster"),
                    ...
                    (String) map.get("slayer.wildernessTaskName"), (int) map.get("slayer.wildernessTaskAmount"),
                    (String) map.get("slayer.wildernessTaskDate"));
        }
        if (map.containsKey("slayer.bossTaskAmount")) {
            bossTask = new AssignedSlayerTask(SlayerMaster.BOSS_MASTER, null, (String) map.get("slayer.bossTaskNamr"),
    ...
    public Object script(String name, Object... args) {
        if (name.equals("slayer_is_any_task")) {
            ...
            var plural = wildernessTask.getQuantity() > 1 ? (wildernessTask.getName().endsWith("ss") ? "es"
                    : (wildernessTask.getName().endsWith("s") ? "'" : "s")) : "";
            return !wildernessTask.isComplete() ? wildernessTask.getQuantity() + " " + wildernessTask.getName() + plural
                    : "none";
        } else if (name.equals("slayer_get_boss_task")) {
            var plural = bossTask.getQuantity() > 1
                    ? (bossTask.getName().endsWith("ss") ? "es" : (bossTask.getName().endsWith("s") ? "'" : "s")) : "";
            return !bossTask.isComplete() ? bossTask.getQuantity() + " " + bossTask.getName() + plural : "none";
    ...
    @Override
    public boolean widgetOnWidgetHook(int useWidgetId, int useChildId, int onWidgetId, int onChildId, int useSlot,
            int useItemId, int onSlot, int onItemId) {
        if (useWidgetId == WidgetId.INVENTORY && onWidgetId == WidgetId.INVENTORY) {
            ...
            var blackMaskId = UseWidgetDecoder.getMatch(useItemId, onItemId, ItemId.BLACK_MASK, ItemId.BLACK_MASK_1,
            ...
            if (blackMaskId != -1) {
                ...
                if (!player.getInventory().hasItem(ItemId.EARMUFFS) || !player.getInventory().hasItem(ItemId.FACEMASK)
            ...
            var blackMaskIId = UseWidgetDecoder.getMatch(useItemId, onItemId, ItemId.BLACK_MASK_I,
    ...
    public void getAssignment(String name) {
        ...
        for (var i = 0; i < 128 && selectedTask == null; i++) {
            ...
            if (aTask.getIdentifier() == SlayerTaskIdentifier.RED_DRAGON && !isUnlocked(SlayerUnlock.SEEING_RED)) {
                continue;
            }
            ...
            if (aTask.getIdentifier() == SlayerTaskIdentifier.AVIANSIE && !isUnlocked(SlayerUnlock.WATCH_THE_BIRDIE)) {
                continue;
            }
            if (aTask.getIdentifier() == SlayerTaskIdentifier.TZHAAR && !isUnlocked(SlayerUnlock.HOT_STUFF)) {
                continue;
            }
        ...
        if (selectedTask.getIdentifier() == SlayerTaskIdentifier.DARK_BEAST
                ...
                || selectedTask.getIdentifier() == SlayerTaskIdentifier.GARGOYLE && isUnlocked(SlayerUnlock.GET_SMASHED)
                || selectedTask.getIdentifier() == SlayerTaskIdentifier.NECHRYAEL
                        && isUnlocked(SlayerUnlock.NECHS_PLEASE)
                || selectedTask.getIdentifier() == SlayerTaskIdentifier.CAVE_KRAKEN && isUnlocked(SlayerUnlock.KRACK_ON)
        ...
        var plural = assignedTask.getQuantity() > 1
                ? (assignedTask.getName().endsWith("ss") ? "es" : (assignedTask.getName().endsWith("s") ? "'" : "s"))
                : "";
        if (isWilderness) {
            player.getGameEncoder().sendMessage(
                    "Your new wilderness task is to kill " + quantity + " " + selectedTask.getName() + plural + ".");
            player.getGameEncoder().sendMessage(
                    "Carrying a mysterious emblem during your task will reward you with blood money for every kill.");
        } else {
            player.getGameEncoder()
                    .sendMessage("Your new task is to kill " + quantity + " " + selectedTask.getName() + plural + ".");
        }
    ...
    private boolean countsTowardTaskQuantity(AssignedSlayerTask assignedTask, int id) {
        ...
        if ((id == NpcId.DAWN_228 || id == NpcId.DAWN_228_7885) && !isUnlocked(SlayerUnlock.DOUBLE_TROUBLE)) {
            return false;
        }
        if (assignedTask == bossTask) {
            if (id == NpcId.WINGMAN_SKREE_143 || id == NpcId.FLOCKLEADER_GEERIN_149 || id == NpcId.FLIGHT_KILISA_159) {
                return false;
    ...
    public void sendTask() {
        ...
        if (!task.isComplete()) {
            player.getGameEncoder().sendMessage(
                    "You're assigned to kill " + task.getName() + "s; only " + task.getQuantity() + " more to go.");
            if (task.getSlayerTask().getLocation() != null) {
                player.getGameEncoder().sendMessage("They are located at " + task.getSlayerTask().getLocation() + ".");
            }
        }
        if (!wildernessTask.isComplete()) {
            ...
            if (wildernessTask.getSlayerTask().getLocation() != null) {
                player.getGameEncoder()
                        .sendMessage("They are located at " + wildernessTask.getSlayerTask().getLocation() + ".");
            }
        }
    }
    ...
    private void sendVarps() {
        ...
        player.getGameEncoder().setVarp(VarpId.SLAYER_TASK_IDENTIFIER,
                !task.isComplete() ? task.getIdentifier() != null ? task.getIdentifier().ordinal() : 156 : 0);
