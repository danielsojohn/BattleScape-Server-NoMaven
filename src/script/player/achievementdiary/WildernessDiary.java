package script.player.achievementdiary;

import static com.palidino.osrs.model.player.AchievementDiaryTask.Difficulty.EASY;
import static com.palidino.osrs.model.player.AchievementDiaryTask.Difficulty.ELITE;
import static com.palidino.osrs.model.player.AchievementDiaryTask.Difficulty.HARD;
import static com.palidino.osrs.model.player.AchievementDiaryTask.Difficulty.MEDIUM;
...
import com.palidino.osrs.model.item.ShopItem;
...
public class WildernessDiary extends AchievementDiary {
    ...
    public AchievementDiaryTask[] getTasks() {
        return Arrays.stream(WildernessTask.values()).map(WildernessTask::getTask).toArray(AchievementDiaryTask[]::new);
    }
...
enum WildernessTask {
    CAST_ALCHEMY(new AchievementDiaryTask("Cast High Alchemy at the Fountain of Rune.", MEDIUM)), // not done
    LEVER(new AchievementDiaryTask("Enter the Wilderness from the Ardougne or Edgeville lever", MEDIUM)),
    WILDERNESS_ALTAR(new AchievementDiaryTask("Pray at the Chaos Temple in level 38, Western Wilderness", MEDIUM)),
    CHAOS_TEMPLE(new AchievementDiaryTask("Enter the Chaos Runecrafting temple.", MEDIUM)), // not done
    MAMMOTH(new AchievementDiaryTask("Kill a mammoth.", EASY)),
    EARTH_WARRIOR(new AchievementDiaryTask("Kill an earth warrior in the Wilderness beneath Edgeville.", EASY)),
    DEMONIC_RUINS(new AchievementDiaryTask("Restore some Prayer points at the Demonic Ruins.", MEDIUM)), // not done
    KING_BLACK_DRAGON(new AchievementDiaryTask("Enter the King Black Dragon Lair.", MEDIUM)),
    SPIDER_EGGS(new AchievementDiaryTask("Collect 5 red spider's eggs from the Wilderness.", MEDIUM)), // not done
    IRON_ORE(new AchievementDiaryTask("Mine some iron ore in the Wilderness.", EASY)),
    MAGE_OF_ZAMORAK(new AchievementDiaryTask("Have the Mage of Zamorak teleport you to the Abyss.", EASY)),
    TEAM_CAPE(new AchievementDiaryTask("Equip any team cape in the Wilderness.", MEDIUM)), // not done
    ...
    WILDERNESS_GODWARS(new AchievementDiaryTask("Enter the Wilderness God Wars Dungeon", MEDIUM)), // not done
    WILDERNESS_AGILITY(new AchievementDiaryTask("Complete a lap of the Wilderness Agility Course.", MEDIUM)), // not
                                                                                                              // done
    ...
    BLOODVELD(new AchievementDiaryTask("Kill a bloodveld in the Wilderness God Wars Dungeon.", MEDIUM)), // not done
    MYSTERIOUS_EMBLEM(new AchievementDiaryTask(" Sell a mysterious emblem to the Emblem Trader in Edgeville.", MEDIUM)), // not
                                                                                                                         // done
    GOLD_HELMET(new AchievementDiaryTask("Smith a Gold helmet in the Resource Area.", MEDIUM)), // not done
    MUDDY_CHEST(new AchievementDiaryTask("Open the muddy chest in the Lava Maze.", MEDIUM)), // not done

    GOD_SPELL(new AchievementDiaryTask("Cast one of the 3 God spells against another player in the Wilderness.", HARD)),
    ...
    ADAMANT_SCIMITAR(new AchievementDiaryTask("Smith an adamant scimitar in the Resource Area.", HARD)),
    LAVA_DRAGON(new AchievementDiaryTask("Kill a lava dragon and bury the bones on Lava Dragon Isle.", HARD)),
    CHAOS_ELEMENTAL(new AchievementDiaryTask("Kill the Chaos Elemental.", HARD)),
    DEMI_BOSSES(new AchievementDiaryTask(" Kill the Crazy archaeologist, Chaos Fanatic & Scorpia.", HARD)),
    TROLLHEIM_SHORTCUT(new AchievementDiaryTask("Take the Agility Shortcut from Trollheim into the Wilderness.", HARD)),
    SPIRITUAL_WARRIOR(new AchievementDiaryTask("Kill a spiritual warrior in the Wilderness God Wars Dungeon.", HARD)),
    ...
    RUNE_SCIMITAR(new AchievementDiaryTask(" Smith a rune scimitar from scratch in the Resource Area.", ELITE)),
    ROGUES_CASTLE_CHEST(new AchievementDiaryTask("Steal from the Chest (Rogues' Castle).", ELITE)),
    SPIRITUAL_MAGE(new AchievementDiaryTask("Slay a spiritual mage inside the Wilderness God Wars Dungeon.", ELITE)),
