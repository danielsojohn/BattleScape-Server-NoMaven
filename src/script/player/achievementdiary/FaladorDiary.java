...
public class FaladorDiary extends AchievementDiary {
    ...
    public AchievementDiaryTask[] getTasks() {
        return Arrays.stream(FaladorTask.values()).map(FaladorTask::getTask).toArray(AchievementDiaryTask[]::new);
    }
...
enum FaladorTask {
    ...
    SECURITY_BOOK(new AchievementDiaryTask("Claim a security book from the Security guard upstairs at Port Sarim jail.",
            EASY)),
    ...
    GUTHIX_ALTAR(
            new AchievementDiaryTask("Pray at the Altar of Guthix in Taverley whilst wearing full Initiate.", MEDIUM)),
    ...
    CAPE_EMOTE(
            new AchievementDiaryTask("Perform a skillcape or quest cape emote at the top of Falador Castle.", ELITE)),
