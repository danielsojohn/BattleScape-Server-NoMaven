...
public class TrainingDayDiary extends AchievementDiary {
    ...
    public void makeItem(Player player, int skillId, Item item, Npc npc, MapObject mapObject) {
        ...
        if (skillId == Skills.THIEVING && mapObject != null && mapObject.getName().toLowerCase().endsWith("stall")) {
        ...
        if (skillId == Skills.FARMING
                && (item.getId() == ItemId.GRIMY_GUAM_LEAF || item.getId() == ItemId.GRIMY_GUAM_LEAF_NOTED)) {
