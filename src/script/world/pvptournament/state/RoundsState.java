    ...
    public void lobbyTick() {
        for (var player : tournament.getPlayers()) {
            ...
            for (var brewDeleteId : brewIds) {
                ...
                player.getInventory().deleteItem(brewDeleteId, brewCount - tournament.getMode().getBrewCap());
            }
        }
        if (timer > 0) {
            return;
        ...
        for (var i = 0; i < tournament.getPlayers().size(); i += 2) {
            ...
            if (i == tournament.getPlayers().size() - 1) {
                player1.getGameEncoder().sendMessage("<col=ff0000>You don't have an opponent for this round.");
                break;
            }
