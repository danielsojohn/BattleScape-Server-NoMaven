...
public class LvlCommand implements Command {
    ...
    public void execute(Player player, String command) {
        ...
        player2.getGameEncoder().sendMessage(
                player.getUsername() + " has adjusted your " + Skills.SKILL_NAMES[skillID] + " to " + skillLevel);
        ...
        if (player2.getSkills().getLevelForXP(skillID) < skillLevel) {
            player2.getSkills().addXp(skillID, Skills.XP_TABLE[skillLevel] - player2.getSkills().getXP(skillID), false);
        } else {
