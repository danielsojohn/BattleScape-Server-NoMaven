...
public class PestilentBloatCombat extends NpcCombat {
    ...
    private void checkLineOfSight() {
        ...
        for (var i = 0; i < players.size(); i++) {
            ...
            if (!this.inArea(player) || !npc.withinDistance(player, 4)
                    && (player.getX() < minX || player.getX() > maxX || player.getY() < minY || player.getY() > maxY)) {
                continue;
            }
            ...
            for (var i2 = 0; i2 < players.size(); i2++) {
                ...
                var lineOfSight = this.inNorth(player) && this.inNorth(player2)
                        || this.inEast(player) && this.inEast(player2) || this.inSouth(player) && this.inSouth(player2)
