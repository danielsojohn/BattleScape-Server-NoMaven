...
public class VerzikViturCombat extends NCombatScript {
    ...
    public List<NpcCombatDefinition> getCombatDefs() {
        ...
        combat1.hitpoints(NpcCombatHitpoints.builder().total(2000).bar(HitpointsBar.GREEN_RED_100).build());
        combat1.stats(NpcCombatStats.builder().attackLevel(400).magicLevel(400).rangedLevel(400).defenceLevel(20)
        ...
        combat2.hitpoints(NpcCombatHitpoints.builder().total(3250).bar(HitpointsBar.GREEN_RED_100).build());
        combat2.stats(NpcCombatStats.builder().attackLevel(400).magicLevel(400).rangedLevel(400).defenceLevel(200)
                .bonus(CombatBonus.ATTACK_MAGIC, 80).bonus(CombatBonus.ATTACK_RANGED, 80)
    ...
    public void tick() {
        ...
        if (npc.getId() == NpcId.VERZIK_VITUR_1040_8370) {
            phase1Tick();
        } else if (npc.getId() == NpcId.VERZIK_VITUR_1040_8371 || npc.getId() == NpcId.VERZIK_VITUR_1265) {
            phase2Tick();
        }
    }
    ...
    private void phase1Tick() {
        ...
        for (var entity : hitEntities) {
            var projectile = Graphic.Projectile.builder().id(1580).entity(entity).startTile(npc)
                    .projectileSpeed(npc.getCombat().getSpeed(null, entity, HitType.MAGIC, 10)).hitType(HitType.MAGIC)
                    .build();
    ...
    private void phase2Tick() {
        ...
        if (Utils.getPercent(npc.getHitpoints(), npc.getMaxHitpoints()) < 35) {
            if (redSpiderDelay-- <= 0) {
                ...
                var redSpider2 = new Npc(npc.getController(), NpcId.NYLOCAS_MATOMENOS_115_8385, new Tile(3172, 4314));
