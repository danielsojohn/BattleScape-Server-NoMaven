...
public class EventChaosElementalCombat extends NpcCombat {
    ...
    public List<NpcCombatDefinition> getCombatDefinitions() {
        ...
        combat.hitpoints(NpcCombatHitpoints.builder().total(2000).bar(HitpointsBar.GREEN_RED_120).build());
    ...
    public void npcApplyHitEndHook(Hit hit) {
        ...
        fanatics.add(
                new Npc(npc.getController(), NpcId.CHAOS_FANATIC_202, npc.getX() - 1, npc.getY() + 2, npc.getHeight()));
        fanatics.add(
                new Npc(npc.getController(), NpcId.CHAOS_FANATIC_202, npc.getX() + 1, npc.getY() - 2, npc.getHeight()));
    ...
    public double damageReceivedHook(Entity opponent, double damage, HitType hitType, HitType defenceType) {
        ...
        var player = (Player) opponent;
        if ((!npc.getController().inMultiCombat() || !opponent.getController().inMultiCombat()) && damage > 6) {
            damage = 6;
        }
        if (player.getEquipment().wearingFullVoid(HitType.MELEE)
                || player.getEquipment().wearingFullVoid(HitType.RANGED)
