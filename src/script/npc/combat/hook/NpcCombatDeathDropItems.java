...
public class NpcCombatDeathDropItems implements NpcCombatGlobalHooks {
    public void deathDropItems(Npc npc, Player player, int index, boolean isSlayerTask, boolean isWildernessSlayerTask,
        ...
        if (npc.getController().inWilderness() && (npc.getDef().getCombat().getKillCount().isSendMessage()
                || isWildernessSlayerTask || npc.getDef().getName().contains("Revenant")) && PRandom.randomE(300) == 0) {
            ...
            if (player.getCombat().getBountyHunter().hasEmblem()) {
                ...
                if (PRandom.randomE(8) == 0) {
                    npc.getController().addMapItem(
                            new Item(PRandom.randomI(1) == 0 ? ItemId.DARK_CRAB : ItemId.PRAYER_POTION_3), dropTile,
                            player);
                }
            }
            if (npc.getMaxHitpoints() / 2 < 155 && PRandom.randomE(155 - npc.getMaxHitpoints() / 2) == 0) {
                npc.getController().addMapItem(new Item(ItemId.MYSTERIOUS_EMBLEM), dropTile, player);
            }
        ...
        if (npc.getController().inWilderness() && PRandom.randomE(30) == 0 && !player.hasItem(ItemId.LOOTING_BAG)) {
