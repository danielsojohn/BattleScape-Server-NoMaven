...
public class CerberusCombat extends NpcCombat {
    private static final NpcCombatStyle SPECIAL_ATTACK = NpcCombatStyle.builder().type(NpcCombatStyleType.MELEE_SLASH)
            .damage(NpcCombatDamage.maximum(23)).animation(4491).attackSpeed(4).attackRange(10).build();
    ...
    public int attackTickAttackSpeedHook(NpcCombatStyle combatStyle) {
        if (tripleDelay == TRIPLE_DELAY || tripleDelay == TRIPLE_DELAY - 1 || tripleDelay == TRIPLE_DELAY - 2) {
            return 2;
        }
    ...
    private void startSouls() {
        ...
        if (npc.getRegionId() == 4883) {
            tiles = new Tile[] {
                new Tile(1239, 1265), new Tile(1240, 1265), new Tile(1241, 1265)
            };
        } else if (npc.getRegionId() == 5395) {
            tiles = new Tile[] {
                new Tile(1367, 1265), new Tile(1368, 1265), new Tile(1369, 1265)
            };
        } else if (npc.getRegionId() == 5140) {
            tiles = new Tile[] {
                new Tile(1303, 1329), new Tile(1304, 1329), new Tile(1305, 1329)
            };
        }
        if (tiles == null) {
            return;
    ...
    private void checkAOE() {
        if (aoeDelay > 0) {
            ...
            for (var aoe : aoeAttacks) {
                ...
                if (!withinDistance || entity.isLocked() || aoe.tick - AOE_LENGTH + 2 >= aoe.speed.getEventDelay()) {
                    continue;
                }
    ...
    private void startAOE() {
        ...
        aoeAttacks.clear();
        var tiles = new Tile[] {
            new Tile(entity), new Tile(entity).randomize(2), new Tile(entity).randomize(4)
        };
        for (var tile : tiles) {
